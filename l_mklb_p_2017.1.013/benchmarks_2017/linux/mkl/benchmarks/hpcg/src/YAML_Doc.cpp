...
extern __attribute__((target(mic))) char *yamlReport, *yamlReportFile;
#endif
...
string YAML_Doc::generateYAML() {
  ...
  if (destinationFileName=="")
    filename = miniAppName + "-" + miniAppVersion + "_" + string(sdate);
  else
...
#ifdef HPCG_OFFLOAD
  strncpy(yamlReportFile, filename.c_str(), FILENAME_MAX_LEN);
  strncpy(yamlReport, yaml.c_str(), CONTENTS_MAX_LEN);
#else
  ofstream myfile;
