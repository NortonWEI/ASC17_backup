...
void GenerateCoarseProblem(const SparseMatrix & Af, const HPCG_Params& params, int level) {
  ...
  local_int_t * f2cOperator =
    (local_int_t *)_mm_malloc(sizeof(local_int_t)*Af.localNumberOfRows, 64);
...
#ifndef NDEBUG
  Ac->localToGlobalMap = new global_int_t[geomc->nx*geomc->ny*geomc->nz];
#endif
  GenerateProblem(*Ac, 0, 0, 0, params);
  SetupHalo(*Ac);
...
#ifdef HPCG_OFFLOAD
  if (1 == level) {
    gbl_offload_m1 = Ac->localNumberOfRows;
    gbl_offload_n1 = Ac->localNumberOfColumns;
  }
  else if (2 == level) {
    gbl_offload_m2 = Ac->localNumberOfRows;
    gbl_offload_n2 = Ac->localNumberOfColumns;
  }
  else if (3 == level) {
    gbl_offload_m3 = Ac->localNumberOfRows;
    gbl_offload_n3 = Ac->localNumberOfColumns;
  }
  else {
    assert(false);
  }

  gbl_offload_level = level;
  gbl_offload_signal = HPCG_OFFLOAD_SET_PROBLEM_SIZE;
  while (HPCG_OFFLOAD_RUN != gbl_offload_signal) {};

  if (1 == level) {
    SetVector(*xc, Ac->localNumberOfColumns, gbl_offload_x1);
  }
  else if (2 == level) {
    SetVector(*xc, Ac->localNumberOfColumns, gbl_offload_x2);
  }
  else if (3 == level) {
    SetVector(*xc, Ac->localNumberOfColumns, gbl_offload_x3);
  }
  else {
    assert(false);
  }
...
#endif
#ifndef HPCG_NO_MPI
  SetHaloCommId(*xc, HPCG_OFFLOAD_VECTOR_X1 - 1 + level);
#endif
  InitializeVector(*Axf, Af.localNumberOfColumns);
  Af.Ac = Ac;
  MGData * mgData = new MGData;
  InitializeMGData(f2cOperator, rc, xc, Axf, *mgData);
  mgData->numberOfPresmootherSteps = params.numberOfPresmootherSteps;
  mgData->numberOfPostsmootherSteps = params.numberOfPostsmootherSteps;
  Af.mgData = mgData;
